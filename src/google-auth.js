...
function scopes() {
  ...
  if (process.env.GOOGLE_GMAIL_SCOPE === 'true') base.push('https://www.googleapis.com/auth/gmail.readonly');
  return base;
}

function sign(value) {
  return crypto.createHmac('sha256', required('GOOGLE_SESSION_SECRET')).update(value).digest('base64url');
}
...
export function googleAuthStatus() {
  return {
    ...
    scopes: scopes(),
    gmailAccess: process.env.GOOGLE_GMAIL_SCOPE === 'true',
    session: 'signed HttpOnly cookie',
    secretStoredIn: 'Render environment variables'
  };
}

export function createGoogleAuthorization(req, res) {
  required('GOOGLE_CLIENT_ID');
  required('GOOGLE_SESSION_SECRET');
  const state = crypto.randomBytes(32).toString('base64url');
  const statePayload = pack({ value: state, exp: Date.now() + STATE_TTL_MS });
  res.cookie?.(STATE_COOKIE, statePayload, { httpOnly: true, secure: true, sameSite: 'lax', maxAge: STATE_TTL_MS, path: '/api/auth/google' });
...
export function validateGoogleState(req, state) {
  const payload = unpack(req.cookies?.[STATE_COOKIE]);
  return Boolean(payload && payload.value === state);
}
...
export function createSessionCookie(profile) {
  const payload = { sub: profile.sub, email: profile.email, name: profile.name, picture: profile.picture, exp: Date.now() + SESSION_TTL_SECONDS * 1000 };
  return pack(payload);
}

export function readSession(req) {
  return unpack(req.cookies?.[SESSION_COOKIE]);
}
