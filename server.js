...
app.get('/api/status', (req, res) => res.json({
  ...
  eyeLens: { status: 'ready', protocol: 'gaze', cameraFeed: 'external-client-signal' }
}));
...
app.get('/api/governance/constitution', (req, res) => res.json({
  ...
  safeguards: ['No safety bypass', 'No privacy bypass', 'No security bypass', 'No viewpoint suppression', 'Human review remains available for ambiguous cases']
}));
...
app.post('/api/governance/evaluate', (req, res) => {
  try {
    ...
    const result = processInput({ ...req.body, type: req.body?.type === 'web' ? 'web' : 'post', source: `governance:${actor}` });
    res.status(200).json({ ok: true, governance: 'MercySoul Dominion Constitution', equalTreatment: true, actor, ...result });
  } catch (error) {
...
app.post('/api/verify', async (req, res) => {
  res.json({ success: true, governanceBound: true, text: 'MercySoul verification ready - test: ' + (req.body.prompt || '') });
});
