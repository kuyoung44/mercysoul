...
function ensureAse(text, includeWhatsApp = false) {
  ...
  if (includeWhatsApp && !cleaned.includes(WHATSAPP_URL)) cleaned = `${cleaned}\n\n${WHATSAPP_LINK}`;
...
function crmResponse(message) {
  ...
  if (upper === 'MENU' || upper.startsWith('MENU ')) return ensureAse([
    ...
    '• MercySoul News Gate (Company Site) — ₦75,000',
    '• MercySoul Enterprise Framework — ₦750,000',
    '', 'Tell me what your business needs and I will recommend the best option.', '', WHATSAPP_LINK,
  ].join('\n'));
...
export default async function handler(req, res) {
  ...
  else if (documentUri) return res.status(400).json({ reply: 'Direct document URI mode is no longer supported. Upload the PDF with the Upload Document control. Aṣẹ.' });
  ...
  try {
    ...
    if (!response) return res.status(502).json({ reply: 'MercySoul is having trouble connecting to its AI service right now. Please try again in a moment. Aṣẹ.' });
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 || response.status === 503) return res.status(503).json({ reply: 'MercySoul is experiencing unusually high AI traffic right now. Please try again in a moment. Aṣẹ.' });
    ...
    return res.status(503).json({ reply: 'MercySoul is temporarily unable to connect to its AI service. Please try again in a moment. Aṣẹ.' });
  }
}
