...
export default async function handler(req, res) {
  ...
  if (!validateMessage(message)) {
    return res.status(400).json({ reply: 'Please provide a message of 1–4000 characters. Aṣẹ.' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[MercySoul Security]', JSON.stringify({ event: 'gemini_error', status: response.status, model: MODEL, requestId: context.requestId }));
    ...
    console.error('[MercySoul Security]', JSON.stringify({ event: 'gemini_request_failed', requestId: context.requestId, error: String(error?.message || error) }));
